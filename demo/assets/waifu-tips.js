...
function showMessage(text, timeout, flag) {
    if(flag || sessionStorage.getItem('waifu-text') === '' || sessionStorage.getItem('waifu-text') === null){
        ...
        $('.waifu-tips').finish().html(text).fadeTo(fadeTime, 1);
...
function hideMessage(timeout) {
    ...
    $('.waifu-tips').delay(timeout).fadeTo(fadeTime, 0);
}
...
var hitokotoFromList = [];

function clearHitokoto() {
    for (var id of hitokotoFromList) {
        clearTimeout(id);
    }
}
    	    ...
    	    default:
    	        $.getJSON('https://v1.hitokoto.cn',function(result){
            	    if (!empty(result.from)) {
                        ...
                        var timeoutID = window.setTimeout(function() {showMessage(text, 3000, true);}, 5000);
                        clearHitokoto();
                        hitokotoFromList.push(timeoutID);
                    }
                    showMessage(result.hitokoto, 5000, true);
                });
    	}
