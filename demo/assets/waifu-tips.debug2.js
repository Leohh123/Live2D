...
function showMessage(text, timeout, flag) {
    if(flag || sessionStorage.getItem('waifu-text') === '' || sessionStorage.getItem('waifu-text') === null){
        ...
        cnt++;
        var nowcnt = String(cnt);
        $('.waifu-tips').stop(true);
        $('.waifu-tips').html(text).fadeTo(200, 1, () => {
            lastTime = new Date().getTime();
            console.log("show_f", nowcnt, lastTime);
        });
        if (timeout === undefined) timeout = 5000;
        hideMessage(timeout, nowcnt);
    }
}

function hideMessage(timeout, nowcnt) {
    // $('.waifu-tips').stop().css('opacity',1);
    if (timeout === undefined) timeout = 5000;
    window.setTimeout(function() {sessionStorage.removeItem('waifu-text')}, timeout);
    $('.waifu-tips').delay(timeout, "fx").fadeTo(200, 0, () => {
        console.log("hide_f", nowcnt, "      " , new Date().getTime() - lastTime);      
    });
}
